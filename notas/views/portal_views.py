...
def portal_vista(request):
    ...
    Ahora, esta vista depende del colegio identificado por el middleware.
    """
    # Si el middleware no encontró un colegio para este dominio, muestra un error.
    if not request.colegio:
        return HttpResponseNotFound("<h1>Este dominio no está asociado a ningún colegio.</h1>")

    # Se procesa el formulario de login si el método es POST.
    ...
    # Pasamos el objeto colegio a la plantilla para personalizarla.
    context = {
...
def directorio_docentes_json(request):
    """
    Crea una lista en formato JSON con la información de los docentes
    DEL COLEGIO ACTUAL.
    """
    if not request.colegio:
    ...
    try:
        ...
        docentes = Docente.objects.filter(colegio=request.colegio, user__is_active=True).prefetch_related(
            Prefetch(
                ...
                queryset=Asignacion.objects.filter(colegio=request.colegio).select_related('curso', 'materia').order_by('curso__nombre', 'materia__nombre'),
...
def ajax_historia(request):
    # Esta vista podría necesitar personalización por colegio en el futuro
